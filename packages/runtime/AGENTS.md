...
- 当前 Run checkpoint API 由 host-side shadow Git 提供：`create_base`、`seal`、`status`、`gc`，以及 `revert_preview`/`revert_execute`/`revert_blob`/`git_status`。shadow repo 位于 host root 的 `.xihe-shadow/<workspaceId>.git`，Runtime 持有 refs；用户仓库的 `.git`、index、branch 不被写入。V22 是 CP 对 checkpoint 的 durable projection，V23 是 revert bookkeeping；CP 不是 Runtime refs 的权威。
- Checkpoint 建立时按 workspace 原子取得 mutation lease；默认 TTL 30 分钟，同一 run 可续租，其他 run 返回 lease-held/409，Runtime 重启会丢弃内存 lease 但保留持久 refs。host Git 不可用、版本过旧或 workspace 不明时返回显式 degraded/unavailable（不静默回退）；seal 后仅 best-effort retention GC。
