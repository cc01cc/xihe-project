...
/**
 * PLAN-0328 M2 W3: CP-side projection of a Run checkpoint owned by the Runtime
 * (shadow git). One row per {@code (run_id, workspace_id)}; the Runtime remains
 * authoritative for the checkpoint refs themselves.
 *
 * <p>The CP row is the durable lifecycle record the terminal-transition hooks and
 * the startup recovery sweep work on. {@code changed_files} keeps the seal-time
 * change set as JSON text ({@code [{"status":"M","path":"a.txt"}]}).</p>
 */
@Entity
...
public class RunCheckpoint {
    ...
    /** Baseline established; the Run may still mutate the workspace. */
    public static final String STATE_BASE = "base";
    /** End ref written; the change set is frozen. */
    public static final String STATE_SEALED = "sealed";
    /** Base exists but the end ref is missing (Runtime sweep pending). */
    public static final String STATE_UNSEALED = "unsealed";
    /** Establishment failed; {@code unrollableReason} explains why. */
    ...
    private String state = STATE_BASE;
