 ...
 *
 * <p><b>Fail-closed</b>: any load failure is logged and yields {@link PolicyContext#EMPTY}
 * (⇒ default ask), never allow (spec §4.3).</p>
 */
@Component
@Primary
public class DbPolicyContextProvider implements PolicyContextProvider {
    ...
    public PolicyContext load(String userId, String workspaceId, String sessionId) {
        try {
            ...
            layers.add(layer(PolicyLayer.INSTANCE, ruleRepository.findByLayerAndOwnerIdIsNullOrderByCreatedAtAscIdAsc(LAYER_INSTANCE)));
            if (userId != null) {
                layers.add(layer(PolicyLayer.USER, ruleRepository.findByLayerAndOwnerIdOrderByCreatedAtAscIdAsc(LAYER_USER, userId)));
            }
            if (workspaceId != null) {
                layers.add(layer(PolicyLayer.WORKSPACE, ruleRepository.findByLayerAndOwnerIdOrderByCreatedAtAscIdAsc(LAYER_WORKSPACE, workspaceId)));
            }
            ...
            return new PolicyContext(layers, loadFaces(workspaceId), mode, modeLayer);
        } catch (RuntimeException e) {
            // fail-closed: an unreadable rule set must never relax the decision
            log.error("[POLICY] context load failed, falling back to built-in only (fail-closed) "
