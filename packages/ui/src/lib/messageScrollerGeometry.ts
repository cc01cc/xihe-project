...
} from './messageScrollerTypes'

export interface ScrollableGeometryInput {
...
export function getMessageScrollerScrollable({
  ...
  spacer,
  viewport,
}: ScrollableGeometryInput): MessageScrollerScrollable {
  if (!viewport || !content) {
...
export interface VisibilityGeometryInput {
  content: HTMLElement | null
  scrollMargin: number
...
export function getMessageScrollerVisibilityState({
  ...
  viewport,
  visibleMessageIds,
}: VisibilityGeometryInput): MessageScrollerVisibilityState {
  if (!content || !viewport) {
...
export function hasMultipleNewScrollAnchors(
  items: HTMLElement[],
...
export function getElementScrollTop({
  ...
  if (align === 'center') {
    ...
    return (
      elementTop - contentPadding.start - (insetHeight - elementHeight) / 2 - scrollMargin
    )
  }
...
export function getContentBlockPadding(spacer: HTMLElement | null) {
