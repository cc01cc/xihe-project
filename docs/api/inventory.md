...
- **Durable binding (V20)**: `approval_requests` carries `policy_revision` (epoch micros of `max(updated_at)` over `policy_rules` + `tool_faces`, `0` when both are empty), `sandbox_generation` (`Workspace.generation` at decision time) and `reuse_scope` (`once|session|saved`; legacy NULL rows are treated as `once`, fail-closed). A plain `(session_id, tool, arguments_hash)` index backs the gate's idempotent lookup.
- **Consume checks (all fail-closed)**: a grant is consumed once and only while unexpired, with `rank(modeAtGrant) >= rank(currentMode)` (`plan > managed > default > accept-edits > bypass`; NULL mode ranks as `default`), an unchanged durable `policy_revision`, and a matching `sandbox_generation`. Legacy rows without the V20 columns are rejected. The `approval_grant_consumed` audit record is emitted inside the consume transaction (R7).
...
- **Ledger**: checkpoint creation/seal appends an operation-ledger marker (`kind=checkpoint`, `source=runtime`, `tool_name=run_checkpoint`) and UI-triggered revert appends `kind=checkpoint`, `source=ui`, `tool_name=revert_snapshot`; when the run has no durable operation the marker is skipped and logged (never fabricated).
