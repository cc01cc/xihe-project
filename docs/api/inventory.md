...
| CP | `/workspaces/current`, `/workspaces`, `/workspaces/{workspaceId}` | unchanged under `/api/v1/workspaces/...`; `GET /api/v1/workspaces` lists all active Workspace memberships, `POST` creates another active Workspace with optional `storageMode/hostPath/executionMode` and `Idempotency-Key` | user Bearer | UI Workspace selector, Runtime lifecycle |
| CP | Workspace execution-mode switch | `PATCH /api/v1/workspaces/{workspaceId}/execution-mode` | Workspace owner/admin Bearer | UI Environment view; rejects active jobs and never changes storageMode |
| CP | Workspace durable Job projection | `GET /api/v1/workspaces/{workspaceId}/jobs` | Workspace access Bearer | Scope/status view; canonical identity is `operationItemId` |
...
| Runtime | direct-attach capability probe | `POST /internal/v1/runtime/capabilities/direct-attach/probe` | service Bearer | CP create/switch preflight; returns `contractVersion: v1`, backend identity, maturity, available/reason |
