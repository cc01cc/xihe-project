...
- **Ledger**: checkpoint creation/seal appends an operation-ledger marker (`kind=checkpoint`, `source=runtime`, `tool_name=run_checkpoint`) and revert appends `kind=checkpoint`, `source=cp`, `tool_name=revert_snapshot`; when the run has no durable operation the marker is skipped and logged (never fabricated).
