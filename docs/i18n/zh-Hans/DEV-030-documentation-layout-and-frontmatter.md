...
| 01x 模块 | DEV-010~019 | UI（010 架构/011 清单/012 参考）、Agent（013）、CP（014）、Runtime（015）、MCP（016）、Session（017）、Known Issues（018）、数据库设计（019） |
| 02x 测试 | DEV-020~023 | E2E、集成、单元、Mock |
| 03x 规范 | DEV-030 | 本文档 |

块内留空位可扩展（如 005-009、024-029）；新文档按主题落块，不得复用已退役编号含义。
...
- 图：按内容选型，不唯 mermaid 论——流程/时序/状态用 mermaid（含 subgraph 承载细节），目录树/层级一览保持 ASCII（mermaid 无更优类型）；转换图必须与原文信息一一对齐（PLAN-241 Decision 7 教训：压缩式转换丢信息）；每图注代码锚点。ASCII 流程图不再新增。
