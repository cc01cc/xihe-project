...
- **UI toast 基础样式缺失（2026-09-13 由 PLAN-0323 验证发现）**：`vue-sonner/style.css`（`lib/index.css`，含 `[data-sonner-toaster]{position:fixed}` 等基础样式）从未被任何入口导入（全仓 grep 无引用；JS bundle 仅设置 `data-sonner-toaster` 属性、无内联 CSS）→ toast 以未样式化 `<li>` 落文档流底部（实测 `rect.y=1080` 视口外、`position: static`），**全应用 toast 反馈实际不可见**。修复：`main.ts` 或 `components/ui/sonner/Sonner.vue` 显式 `import 'vue-sonner/style.css'` 并做浏览器可视化复验；属独立小修。
