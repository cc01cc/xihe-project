...
- **ConfigClient URL 路径**（Agent `config_client.py`、Runtime `config_client.rs`）：CP 内部端点路径为 `/internal/config/{layer}/{domain}`（无 `/api/v1` 前缀）。PLAN-049 T3 测试暴露了此 bug——Agent/Runtime ConfigClient 曾误用 `/api/v1/internal/config/...`。新增模块调用时确认路径不含 `/api/v1/`
