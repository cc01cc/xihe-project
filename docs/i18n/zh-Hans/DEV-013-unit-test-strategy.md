...
| Agent | 18 文件 | ~60 | ~70% | ≥ 80% |
| CP | 26 测试类 | ~90 | ~67% | ≥ 80% |
| Runtime | 10 文件 | ~26 | ~55% | ≥ 70% |
| E2E Mock | 19 spec | ~36 | — | — |
| E2E Real | 12 spec | ~15 | — | — |

> 数据来源：PLAN-020 §1 基线 + 文件扫描更新。UI 9 个 api-integration 测试失败（需后端），E2E 3 mock + 3 real 失败为预存问题。
...
- `vue-router-mock` 用于路由测试
- `fake-indexeddb` 用于 IndexedDB 测试（极少使用，xihe 以 CP 后端为主）
...
| 依赖 | Mock 方式 | 说明 |
|------|----------|------|
| `cpFetch` (API) | `globalThis.cpFetch = vi.fn()` | 拦截所有后端 API 调用 |
| `useToast` | `vi.mock()` 或 spy | 验证 Toast 调用参数 |
| `useRouter` / `useRoute` | `vue-router-mock` | 路由导航测试 |
| `HTMLCanvasElement` | `HTMLCanvasElement.prototype.toBlob = vi.fn()` | 图片压缩测试 |
...
| 依赖 | Mock 方式 | 说明 |
|------|----------|------|
| 外部 HTTP 调用 (Agent/Runtime) | WireMock (T2) 或 Mockito | T2 集成测试用 WireMock |
| HttpServletRequest | `new MockHttpServletRequest()` | Controller 测试 |
| JWT Token | `JwtTokenProvider` 真实实现 | 可以直接用真实 JWT |
| ConfigService | `@MockBean` 或 `Mockito.mock()` | Service 层测试 |
...
| LLM 响应 | mock `litellm.completion` | Chat 功能测试 |
...
| 依赖 | Mock 方式 | 说明 |
|------|----------|------|
| bollard (Docker API) | `mockall` crate | 容器操作 mock |
...
详见 `plans/PLAN-052-unit-test-gap-fill.md` §2.1 完整缺口清单。

### 7.2 已知限制

- UI 9 个 api-integration 测试需要后端，非纯单元测试
- Runtime timeout 功能未实现，对应测试为 `#[ignore]`
...
| P0 | Runtime dotenv_loader | ⏸ 阻塞（模块在 main.rs 非 lib.rs） |
...
详见 `plans/PLAN-052-unit-test-gap-fill.md`。
