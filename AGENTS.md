...
- **UI**: reka-ui + Tailwind v4；聊天组件使用自研 MessageScroller / Message / Bubble / Attachment / Marker 五个组件族；Toast 为唯一反馈渠道
  ...
  - workspace 管理：`POST /api/v1/workspaces` 接受 `storageMode`（`managed_import`/`direct_attach`）+ `executionMode` + `hostPath`（仅 direct-attach）+ `Idempotency-Key`；v1 矩阵固定 `managed_import→docker`（Docker 后置，仅文件导入）、`direct_attach→windows-mxc|windows-host`，`profile`/`image` 仅 docker；创建前用 `POST /api/v1/workspaces/capabilities/preflight` 读真实 backend capability（`available:false`+`reason` 仍是 200，Runtime 不可达 502），UI 卡片不硬编码可用性、MXC 失败禁自动 fallback；`materialize` 异步触发（202）+ 轮询 environment；创建入口 = 空态 + 工具栏/移动 Files sheet「添加 Workspace」共用同一弹窗；托管副本物理目录由 `hostRoot/workspaceId` 派生，direct-attach 使用经 Runtime 校验的显式 `host_directory` binding；同 Workspace 模式切换 `PATCH .../execution-mode`（owner/admin，运行中 Job 409 `WORKSPACE_BUSY`）；执行模式不改变 Workspace 逻辑身份
- **Chat 架构**: Workspace 是独立资源，Session 可选绑定 Workspace；同一工作界面可同时呈现 Chat 与 Workspace，但 Workspace 不依赖 Session。共享的 Session/fileContext 只在 Session 存在时生效；附件仍持久化到后端 Session 专属空间（详见 DEV-001 §7、DEV-017）
  ...
  - ChatRun：按 `(userId, sessionId, Idempotency-Key)` 持久化 run，`origin=user_submission|spawn` 由 CP 写入，公开 `/api/v1/chat` 固定 `user_submission`；内部 spawn 复用父 durable `operation_items.id` 作幂等键、验证 Session provenance 并绕过浏览器 SSE gate；`Message.runId` 关联终态，同 key 不重复启动，ambiguous 只能新 key 手动重试
...
- **配置 key 三方同步（硬约束）**: 新增/修改 config domain key 必须同步四处——（a）CP `config-schemas/*.json`（JSON Schema，同时约束 import 与 UI Settings 保存）、（b）`config.import.example.jsonc` 模板、（c）UI `/settings/config` 表单、（d）UI `stores/config.ts` 的 `INSTANCE_DOMAINS`/`LAYER_DOMAINS.workspace` 域列表；任一漏改会导致 import 与保存**同时 400** 或表单不渲染该域（2026-09-09 logging/user-preference 双 400 实证；第四点随 PLAN-0373 job-policy 于 2026-09-23 补入，DEV-003 同步）。
...
| 层 | 用途 | 文件 | 可运行时修改 |
|----|------|------|------------|
| **环境变量** | 运行前固定（端口/DB/JWT/服务 token/HMAC key/开关） | `.env` → `.env.$XIHE_ENV` → `.env.local`（模块内 dotenv loader 统一加载，后加载覆盖先加载、不覆盖系统 env；`.env`/`.env.dev`/`.env.test`/`.env.prod` 入库且仅非敏感占位值；`.env.local` gitignore 放真实本机值）；`XIHE_MCP_SESSION_ID_HMAC_SECRET` 必需、无 CP 代码默认，生产注入至少 32 随机字节且各 CP 副本一致 | ❌（重启；`--set KEY=VALUE` 为 CLI 最高优先级） |
...
- **进程 Job 引擎（PLAN-0393/0394/0395/0396）**: 非 Docker 执行模式（`windows-host`/`windows-mxc`）的 Workspace Job 由 `job_engine` 承接（Job Object + kill-on-close、有界输出 1 MiB/流 + 15 分钟 TTL、cancel 确认树空）；internal `jobs/cleanup`+`jobs/capabilities` 已冻结，能力经 CP `jobCapability`（environment 响应）透出，UI 按 `canStart && available` 渲染并显式标注无隔离；`windows-mxc` 的 policy 由 `process_guard::build_mxc_policy` 单点生成（缺程序目录 readonlyPaths → 真程序 `0xC0000142`）；MXC 失败一律 fail-closed 不降级宿主。
- **Durable job 档案与续看（PLAN-0344/0390）**: `job_state` extension v1 锚定 tool_call item（按状态机前进 upsert，终态不可回退；`scope` 支持 `run/session/workspace`，缺省 session）；续看 `GET /api/v1/operations/items/{itemId}/job-output`（Workspace access、字节游标、UTF-8 边界由 Runtime `utf8_safe_chunk` 保证、`JOB_OUTPUT_LOST`/`JOB_OUTPUT_EXPIRED` 显式化）；destroy 前 Runtime 枚举存活 job → CP 落 `orphaned`（fail-closed）；运行时限按累计运行时间（paused 不计时）。设计见 DEV-014 §9 与 PLAN-0390。**单 job 取消（PLAN-0366/0390）**：`POST /api/v1/operations/items/{itemId}/cancel`（Workspace access、与续看同键位；经 Runtime internal `jobs/cancel` 复用四阶段终止，僵尸不计存活；已终态幂等 `changed:false`；终止未确认 502 `JOB_CANCEL_UNCONFIRMED` 不改档案；取消 job ≠ 取消 run；写 `operation_events`/`job.cancel`/`actor=user`）。**Workspace Job start（PLAN-0390）**：`POST /api/v1/workspaces/{workspaceId}/jobs`（`Idempotency-Key` 必填，无 Session root 幂等由 V36 部分唯一索引保护）派发 Runtime internal `jobs/start`（无 launcher 显式 501，不 fallback）；Runtime 重启使 `bootId` 变化时对账把未确认 Job 落 `interrupted` 且不重放。
- **Safe Coding Loop**: Runtime mutation core 用 snapshot manifest + pre/post content hash + 多文件 patch 失败回滚；Agent MCP interceptor 经一次性 durable approval grant 恢复批准后 dispatch，grant 缺失/不匹配/重复消费 fail-closed；grant 匹配键 = canonical arguments SHA-256（`arguments_hash`）；Operation preview 仅作有界原文投影，不做正则脱敏/改写，也不参与授权或批准后执行；结构化审计仍用 hash/白名单字段，日志出口继续由序列化层统一脱敏
- **Vue i18n JSON placeholder**: `t()` 消息中不可含 `{...}`
- **MCP session-id 签名**: 必须 HMAC 签名，禁止明文或仅 Base64；密钥由必需环境变量 `XIHE_MCP_SESSION_ID_HMAC_SECRET` 提供，CP 无硬编码 fallback；`dev:host` 每次启动生成临时 key，密钥轮换使现有签名 session-id 失效（客户端重新初始化）
...
- **dev:full/T3 拓扑边界**: 验证主线是 Windows `dev:host`；Compose E2E 的 Runtime docker.sock 挂载与 `XIHE_AGENT_API_TOKEN`/`XIHE_CP_API_TOKEN` 分叉修复已完成（PLAN-0401），容器内 WorkspaceStorage 映射仍缺（`sentinel mismatch`）；该边界未闭合前不得把 `dev:full`/T3 失败归因于 host v1。另：Agent 单 workspace MCP 绑定与“每 E2E 用例独立 workspace”冲突（`main.py:595`）已有 harness 级解法 `ensureAgentWorkspaceBinding`（PLAN-0369/0372，见 `xh-host-e2e-pitfalls` §1）——host 车道已接；compose 车道（`e2e-real.mjs`）未下发重启 env 仍 no-op，compose 全量中 `:43/:44` 类失败归属此缺口，接入为后续任务（2026-09-23 回溯修正，替代原“需产品决策”表述）
- **数据库必须 fresh baseline**: 当前源码迁移目录包含 V1–V41；`V1__init_schema.sql` 是 fresh baseline，V22/V23 是 checkpoint/revert 历史迁移，V27 将 CP 投影重建为 workspace slice rows（旧行物理清空、不做格式迁移），V30–V33 为 PLAN-0351 的 schema 清理与改名（V30 chat/session CHECK、V31 FK 子列索引、V32 删死列、V33 `session_operations`→`ledger_operations` 改名），V34 为 PLAN-0367 的 `operation_extensions` 目标 FK `SET NULL`→`CASCADE` 修复，V35 为 PLAN-0376 的 `workspace_imports`，V36 为 PLAN-0390 的 Workspace Job 幂等索引，V37 为 PLAN-0407 Session provenance，V38 为授权 `grants` 表，V39 为 ChatRun origin 与 spawn 幂等部分唯一索引，V40 为 Session `kind=spawn|fork` 与 provenance all-null/all-present 检查，V41 为 USER/ADMIN 用户及 root Agent Session default grants backfill + audit；`ddl-auto=validate`、`baseline-on-migrate=false`；旧本地库会被拒绝，恢复用 `mise run dev:reset -- -Reset`。`document_chunks` 由 Agent 侧 langchain_postgres 自建，不在 Flyway 链内
